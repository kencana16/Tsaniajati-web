...
<div class="row col-md-12 p-3" style="margin-bottom:100px">
  ...
  <?php foreach($dprod as $dp){ ?>
    ...
    <div class="col-md-6">
      ...
      <?php
      if(!$dp['quantity'] == 0 ){?>
          <?= anchor('cart/add_cart/'.$dp['prod_id'],'<div class="btn btn-md text-white mr-1" style="background:#8a8542"><i class="fas fa-cart-plus"></i> Beli Sekarang</div>')?>
      <?php }else{ ?>
        <div class="text-gray text-center p-1 m-0"><a href="<?= base_url('beranda/preorder/'.$products)?>" class="btn btn-md btn-outline-danger d-block">Pre-Order</a></div>
      <?php } ?>
