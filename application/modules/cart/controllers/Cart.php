...
class Cart extends CI_Controller {
	...
	public function index(){
		$data['judul'] = "TSANIAJATI | Keranjang Belanja";
	...
	function fetch(){
		...
		<div class="table-responsive">
			<table class="table table-bordered">
				<thead class="text-white" style="background-color:#8a8542;">
					<tr>
			...
			foreach($data->result() as $row){
				$output .='
					<tr>
						...
						<td class="text-center">
							'.anchor('cart/min_qty/'.$row->prod_id,'<div class="btn btn-sm text-white mx-auto" style="background-color:#8a8542;"><i class="fas fa-minus"></i></div>').'
							<div id="demo" class="btn btn-sm btn-light mx-auto">'.$row->qty.'</div>
							'.anchor('cart/add_cart/'.$row->prod_id,'<div class="btn btn-sm text-white mx-auto" style="background-color:#8a8542;"><i class="fas fa-plus"></i></div>').'
						</td>
				...
				$output .='
					<tr>
						...
						<td class="text-white font-weight-bold" style="background-color:#8a8542;" align="right" colspan="2">Rp. '.number_format($total_bayar, 0,',','.').' ,-</td>
					</tr>
				';
		}else{
			$output .= '
	...
	public function podelete($order_num){
        $this->db->where('order_num', $order_num);
        $this->db->delete('preeorder');
        redirect('preeorder');
	}

	public function readypo(){
		$config['upload_path']          = './bukti_po/';
		...
		if ($this->upload->do_upload('buktitf')){
			...
			$query = $this->db->query('select * from preeorder where id_user = "'.$id_user.'"')->result_array();
			...
			$data = array(
				...
				'kurir' 		=> $this->input->post('jasakirim'),
			...
			// $data['users']= $this->db->get_where('users', ['username' =>
			// $this->session->userdata('username')])->row_array();
			// $this->session->set_flashdata('message', '<div id="message-center" class="modal fade"><div class="modal-dialog" style=";margin-top:150px;"><div class="modal-content"><div class="row"><img class="mx-auto bg-white" src="'.base_url('assets/dist/gif/done.gif').'" width="150px" style=";margin-top:-70px;border-radius:100px"></div><div class="modal-body"><p class="font-weight-bold text-success">Pesanan Berhasil!</p><p>Terima kasih <span class="font-weight-bold text-success">'.$this->session->userdata('username').'</span> sudah berbelanja di TSANIAJATI. <br>Untuk proses pemesanan akan kami lanjutkan di Email/Whatsapp.</p><br><button class="btn btn-success btn-block" type="button" data-dismiss="modal">OK</button></div></div></div></div>');
			redirect('preeorder');
		}else{
			$error = array('error' => $this->upload->display_errors());
			$_data = array('upload_data' => $this->upload->data());
			$data = array(
				'bukti_img' => 'default.jpg'
			);
			$query = $this->db->insert('orders',$data);
			redirect('preeorder',$error);
		}
	}
