...
class Login extends CI_Controller {
            ...
            if($password == $user->password){
				...
				if($this->session->userdata('verif_akun') == 1){
					if($this->session->userdata('role') == 'admin'){
                        ...
                        // $this->session->set_flashdata('message', '<div id="message-center" class="modal fade"><div class="modal-dialog" style=";margin-top:150px;"><div class="modal-content"><div class="row"><img class="mx-auto bg-white" src="'.base_url('assets/dist/gif/check.gif').'" width="150px" style=";margin-top:-70px;border-radius:100px"></div><div class="modal-body"><p class="font-weight-bold">Selamat datang di TSANIAJATI | My Account.</p><p>Silahkan berbelanja ditoko kami, selamat mencoba menu dan produk kami, dan nantikan diskon yang akan datang!!</p><br><button class="btn btn-warning btn-block" type="button" data-dismiss="modal" >OK</button></div></div></div></div>');
						redirect('user_dashboard');
					}
				}else{
	...
	public function register() {
        ...
        if($valid->run() == false){
            ...
            $this->session->set_flashdata('message', '<div id="message-center" class="modal fade"><div class="modal-dialog" style=";margin-top:150px;"><div class="modal-content"><div class="row"><img class="mx-auto bg-white" src="'.base_url('assets/dist/gif/check.gif').'" width="150px" style=";margin-top:-70px;border-radius:100px"></div><div class="modal-body"><p class="font-weight-bold text-left">Registrasi berhasil!<br><button class="btn btn-block" style="background:#8a8542" type="button" data-dismiss="modal" >OK</button></div></div></div></div>');
            redirect('login');
        }
    }
    ...
    private function _sendEmail($token, $type){
        $config = [
            ...
            'smtp_user'   => 'nisaaicak09@gmail.com',  // Email gmail
            'smtp_pass'   => 'anisya2000',  // Password gmail
            'smtp_crypto' => 'ssl',
            'smtp_port'   => 465,
            'crlf'        => "\r\n",
            'newline'     => "\r\n"
        ...
        $this->email->from('nisaaicak09@gmail.com', 'TSANIAJATI OFFICIAL');
        $this->email->to($this->input->post('email'));

        if($type == 'verify'){
            $this->email->subject('Verifikasi akun | My Account TSANIAJATI');
            $this->email->message(
                'Klik verifikasi akun untuk mengaktifkan <b>My Account TSANIAJATI</b> anda.<br>
